function plotTrajectory3D(time, fps, transl,quats, pts3d)
...
%
%  -time(N): vector with timestamps of the poses
...
assert(numel(time) == size(quats,2));
numPoses = size(transl,2);
assert(numel(time) == numPoses);
